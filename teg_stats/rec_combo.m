function [meanvec, idmat, Nvec, SDvec] = rec_combo(varargin)
...
function [meanvec, idmat, Nvec, SDvec] = rec_combo_inner(depvec, indepvec, meanvec, depth, idmat, idval, Nvec, SDvec)
...
%             newmeanvec = median(selected(~isnan(selected)));
    ...
    for iu = 1:length(u),
        f = find(indepvec(:, depth) == u(iu));
        idval(1, depth) = iu;
        [meanvec, idmat, Nvec, SDvec] = rec_combo_inner(depvec(f), indepvec(f, :), meanvec, depth + 1, idmat, idval, Nvec, SDvec);
    end;
end;
